...
#define SAMPLES_QTY 64 //Signal buffer for one channel
int16_t Square_wave[SAMPLES_QTY];
//int16_t OutputBuffer[SAMPLES_QTY * 2];
/* USER CODE END PD */
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
/* USER CODE END PM */
...
double phase = 2 * PI * FREQUENCY * index / (2 * SAMPLING_FREQ);
Left_out_buffer[index] = (int16_t)(30000 * sin(phase));
...
for (int index = 0; index < SAMPLES_QTY; index++)
{
OutputBuffer[index << 1] = Square_wave[index];
OutputBuffer[(index << 1) + 1] = Square_wave[index];
}
/*** Start Audio play ***/
